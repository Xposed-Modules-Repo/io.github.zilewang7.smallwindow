...
- Diagnostics build: every release also carries `io.github.zilewang7.smallwindow-*_debug_diag.apk`; it writes gesture decisions to `/data/system/smallwindow_filter.log` (root-readable) for issue reports
...
- 诊断版：每个 Release 附带 `io.github.zilewang7.smallwindow-*_debug_diag.apk`，手势决策写入 `/data/system/smallwindow_filter.log`（root 可读），反馈问题请附上该日志
